...
/**
 * Unit Test: CertificateBatch Model — Iterasi 3
 *
 * Lingkup: CertificateBatch (auto batch_token, generateTitle, displayTitle,
 * progressPercent, isDone, batchUrl, relasi) serta relasi Certificate::batch()
 * dari sisi sebaliknya. Sesuai US-13 (Upload File Peserta) & US-14
 * (Generate Massal via Queue).
 *
...
class CertificateBatchModelTest extends TestCase
{
    ...
    public function certificate_belongs_to_batch(): void
    {
        ...
        $this->assertEquals($batch->id, $cert->batch->id);
    }

    #[Test]
    public function certificate_batch_is_null_when_certificate_generated_individually(): void
    {
        $cert = Certificate::factory()->forInstitution($this->institution)->create([
            'batch_id' => null,
        ]);
